...
class FedAvg:
    ...
    def extra_parser(extra_args):
        ...
        parser.add_argument('--hscc', action='store_true',
                            help='whether to use FedDecorr')   # NOTE: original help text says "FedDecorr", but it's for HSCC
        parser.add_argument('--hscc_beta', type=float, default=5,
                            help='beta of the FedHSCC loss')
        parser.add_argument('--hscc_gamma', type=float, default=10,
    ...
    def run_fed(self):
        # Added: list to store per-round time
        round_times = []

        for comm_round in range(self.args.n_comm_round):
            # Added: if timing is enabled, record start time of this round
            ...
            # Added: if timing is enabled, compute and record round duration
            if getattr(self.args, 'measure_time', False):
                round_time = time.perf_counter() - round_start
                round_times.append(round_time)
                self.logger.info(f"Communication round {comm_round+1} time: {round_time:.4f} seconds")

        # Added: after all rounds, output average time and standard deviation
        if getattr(self.args, 'measure_time', False) and len(round_times) > 0:
            ...
            std_time = var ** 0.5
            self.logger.info(f"Average communication time per round: {avg_time:.4f} ± {std_time:.4f} seconds")
            print(f"Average communication time per round: {avg_time:.4f} ± {std_time:.4f} seconds")
